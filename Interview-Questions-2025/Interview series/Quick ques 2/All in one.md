...
## 1. What plugins do you use in Jenkins?

In my projects, I use plugins based on the CI/CD requirements. Commonly, I work with Git or GitHub integration plugins for source-code checkout and webhooks, Pipeline plugins for Jenkinsfile-based automation, Credentials Binding for securely accessing credentials, Docker-related plugins for container builds, Maven Integration for Java/Maven projects, SonarQube Scanner for code-quality analysis, and Kubernetes-related plugins when Jenkins agents are dynamically created in Kubernetes. I try to keep only the required plugins installed and regularly review them because outdated or unnecessary plugins can introduce security and maintenance issues.

---

## 2. What message/event do you get before there is something wrong with a Kubernetes pod?

Kubernetes generates events that help identify what is happening with a pod. I normally check the pod status and then inspect its events using `kubectl describe pod`. For example, `FailedScheduling` can indicate that the scheduler cannot place the pod, `Failed` or container runtime errors can indicate startup problems, `BackOff` events can appear when Kubernetes repeatedly fails to start a container, and image-related events can indicate problems pulling an image. For states such as `CrashLoopBackOff` and `ImagePullBackOff`, I use the events along with container logs to identify the actual root cause.

---

## 3. What is the difference between CrashLoopBackOff and ImagePullBackOff?

`CrashLoopBackOff` means the container was started but repeatedly exited or crashed, so Kubernetes keeps restarting it with an increasing backoff delay. I would check `kubectl logs`, previous container logs, the pod description, environment variables, configuration, probes, and application errors. `ImagePullBackOff` occurs when Kubernetes cannot successfully pull the required container image and is retrying with a backoff. In that case, I check the image name and tag, registry availability, image-pull secrets, node connectivity to the registry, and whether the image actually exists.

---

## 4. If a pod is in Pending state, what will you check?

If a pod is Pending, I first check `kubectl describe pod` and specifically look at the Events section because the scheduler usually provides a reason. I check whether there are enough CPU or memory resources, node selectors, affinity and anti-affinity rules, taints and tolerations, PVC availability, namespace resource quotas, and whether nodes are Ready. I also check whether the pod has scheduling constraints that prevent it from being placed on the available nodes. This helps me determine whether the problem is related to resources, storage, scheduling rules, or node conditions.

---

## 5. If a pod is Running but the application is not accessible, how will you troubleshoot?

I separate the problem into application, container, pod, service, and external-access layers. First, I check pod logs and confirm that the application is actually listening on the expected port. Then I verify the container port, readiness probe, Service selector, endpoints, and Service port/targetPort. I test connectivity from inside the cluster to determine whether the issue is with the application or networking. If the Service works internally but external access fails, I investigate the Ingress or LoadBalancer, security groups, network policies, DNS, and load-balancer configuration.

---

## 6. What happens when a Kubernetes pod gets OOMKilled?

`OOMKilled` means the container was terminated because it exceeded the memory limit assigned to it or the node experienced memory pressure. I first check the pod's resource requests and limits and inspect the previous container's termination reason. I also check application logs and memory usage to determine whether there is a memory leak, an unexpected workload spike, or an incorrectly configured memory limit. Depending on the root cause, I may optimize the application, adjust resource limits appropriately, or investigate node-level memory pressure rather than simply increasing the limit.

---

## 7. How do you check pod logs and events?

For logs, I generally use `kubectl logs <pod-name>` and, when required, specify the container with `-c`. If the container has restarted, I use `kubectl logs <pod-name> --previous` to inspect the logs from the previous instance. For events and detailed pod information, I use `kubectl describe pod <pod-name>`. I also check namespace-level events when troubleshooting scheduling or infrastructure-related problems.

---

## 8. What is the difference between kubectl logs and kubectl describe?

`kubectl logs` shows the output generated by the containerized application, so I use it mainly for application-level troubleshooting such as exceptions, connection errors, or startup failures. `kubectl describe` provides Kubernetes resource information, including container state, probes, mounts, node assignment, conditions, and Events. In a production incident, I usually use both because logs tell me what the application experienced while describe often tells me what Kubernetes was doing around the pod.

---

## 9. How do you troubleshoot a pod that keeps restarting?

I first check the current and previous container logs using `kubectl logs` and `kubectl logs --previous`. Then I use `kubectl describe pod` to check termination reasons, exit codes, events, and probe failures. I verify environment variables, ConfigMaps, Secrets, mounted volumes, application dependencies, resource limits, and liveness/startup probes. If the container is being killed because of memory usage, I investigate `OOMKilled`; if the application exits by itself, I inspect the exit code and application logs. My goal is to identify why the process exits rather than simply increasing restart-related settings.

---

## 10. What are liveness, readiness, and startup probes?

A liveness probe determines whether the application is still healthy and whether Kubernetes should restart the container. A readiness probe determines whether the application is ready to receive traffic; if it fails, Kubernetes removes the pod from the Service endpoints without necessarily restarting it. A startup probe is useful for applications that take a long time to initialize because it gives the application time to start before liveness checks become active. In production, I configure these probes based on actual application startup and health behavior rather than using arbitrary values.

---

## 11. What happens if a readiness probe fails?

When a readiness probe fails, Kubernetes considers the pod not ready to receive traffic. The pod can remain Running, but it is removed from the Service's ready endpoints, so traffic should no longer be routed to it through that Service. Kubernetes does not restart the container just because readiness failed. I would investigate the health endpoint, application startup, dependencies, database connectivity, configuration, and probe timing to understand why it became unready.

---

## 12. How do you troubleshoot a Kubernetes Service that is not routing traffic to pods?

I first check whether the Service exists and whether its selector matches the intended pods. Then I inspect the Service and its endpoints or EndpointSlices to verify that healthy pod IPs are registered. I check the pod labels, readiness status, Service `port` and `targetPort`, and whether the application is actually listening on the target port. If the endpoints are correct but traffic still fails, I investigate NetworkPolicies, kube-proxy/CNI issues, DNS, and the external load balancer if the Service is exposed outside the cluster.

---

## 13. How do you check whether the Service selector matches the pod labels?

I check the Service definition and identify its selector using `kubectl get svc <service-name> -o yaml`. Then I check the labels on the pods using `kubectl get pods --show-labels` or `kubectl get pod <pod-name> -o yaml`. The selector and pod labels must match for Kubernetes to associate those pods with the Service. I also verify the resulting endpoints because matching labels alone are not enough if the pods are not Ready.

---

## 14. What is the difference between ClusterIP, NodePort, and LoadBalancer?

ClusterIP is the default Service type and exposes the application internally within the Kubernetes cluster. NodePort exposes the Service through a port on each node, making it reachable externally if network access to the nodes is allowed. LoadBalancer typically provisions or integrates with an external cloud load balancer and provides external access through that load balancer. In cloud environments, I generally prefer LoadBalancer or an Ingress-based architecture depending on the application's routing requirements.

---

## 15. What happens if a pod is deleted? Who creates it again?

A standalone Pod does not automatically recreate itself when deleted. If the pod is managed by a controller such as a Deployment, the Deployment manages a ReplicaSet, and the ReplicaSet ensures that the desired number of pod replicas exists. Therefore, if one of those pods is deleted, the ReplicaSet creates a replacement pod. This is one reason we normally deploy production workloads through controllers instead of creating individual pods manually.

---

## 16. What is the difference between a Deployment, ReplicaSet, and Pod?

A Pod is the smallest deployable unit in Kubernetes and contains one or more containers that share networking and storage. A ReplicaSet maintains the desired number of pod replicas. A Deployment provides a higher-level mechanism for managing ReplicaSets and supports features such as rolling updates, revision history, scaling, and rollback. In normal application deployments, I use Deployments rather than managing ReplicaSets or Pods directly.

---

## 17. How do you perform a rollback of a Kubernetes Deployment?

Before rollback, I check the deployment history to identify the previous revision. I can use `kubectl rollout history deployment/<deployment-name>` and then perform `kubectl rollout undo deployment/<deployment-name>`. After that, I monitor the rollout using `kubectl rollout status`. In a production environment, I also validate pod health, readiness, application functionality, and traffic after the rollback rather than assuming that the command alone confirms recovery.

---

## 18. How do you check the Deployment rollout status?

I use `kubectl rollout status deployment/<deployment-name>` to monitor whether the new ReplicaSet and pods become ready. I also check `kubectl get deployment`, `kubectl get pods`, and the deployment events when necessary. If the rollout gets stuck, I inspect pod status, readiness probes, image pulls, resource constraints, and deployment events to identify the reason.

---

## 19. What is a ConfigMap and how is it different from a Secret?

A ConfigMap stores non-sensitive configuration such as application settings, environment-specific values, or configuration files. A Secret is intended for sensitive information such as passwords, tokens, and credentials. Both can be consumed as environment variables or mounted as files. I avoid storing sensitive values directly in ConfigMaps or Git repositories and use appropriate secret-management practices for production environments.

---

## 20. How do you update environment variables in a running Kubernetes application?

It depends on how the environment variables are managed. If they come from a ConfigMap or Secret, I update the corresponding resource and then ensure that the workload is restarted or otherwise refreshed so that the new environment is loaded by newly created containers. If the value is directly defined in the Deployment, I update the Deployment specification and Kubernetes performs a rollout when the pod template changes. I then monitor rollout status and validate that the application is using the new configuration.

---

# 🐳 Docker

## 21. What is the difference between CMD and ENTRYPOINT?

`ENTRYPOINT` defines the main executable that the container is intended to run, while `CMD` provides default arguments or a default command. In practice, I use ENTRYPOINT when I want the container to behave like a specific executable and CMD when I want to provide defaults that can be overridden easily. They can also be used together, where ENTRYPOINT defines the executable and CMD supplies its default arguments.

---

## 22. What is a multi-stage Docker build and why would you use it?

A multi-stage Docker build allows me to use separate build and runtime stages in the same Dockerfile. For example, I can compile a Java application in a Maven build image and then copy only the generated JAR into a lightweight runtime image. This keeps build tools and source code out of the final image, reducing image size and attack surface. It also makes deployments faster and improves container security.

---

## 23. How do you reduce the size of a Docker image?

I first inspect the image and identify unnecessary layers and packages. I use a suitable lightweight base image, multi-stage builds, `.dockerignore`, and combine related commands where appropriate. I avoid installing unnecessary tools in the runtime image and copy only the artifacts required to run the application. I also make sure package caches and temporary files are not unnecessarily retained in the final image.

---

## 24. What happens when a Docker container exits?

A container exists as long as its main process, PID 1, is running. When that process exits, the container stops. Docker does not automatically restart it unless a restart policy or an external orchestrator such as Kubernetes is configured to do so. I check the container's exit code and logs to understand why it stopped.

---

## 25. How do you troubleshoot a container that is running but the application is not responding?

I first check the container logs and inspect the running process. Then I verify that the application is listening on the expected port inside the container and that the Docker port mapping is correct. I test connectivity from inside the container and from the host. I also check application configuration, environment variables, dependencies, network connectivity, firewall rules, and whether the application is bound only to localhost instead of `0.0.0.0`.

---

## 26. What is the difference between a Docker volume and bind mount?

A Docker volume is managed by Docker and is generally preferred when I want persistent container data without managing the host path directly. A bind mount maps a specific host filesystem path into the container, giving me more direct control over the host files. Volumes are commonly used for application persistence, while bind mounts can be useful during development or when a specific host directory must be shared.

---

## 27. How do you check container logs?

I use `docker logs <container-name-or-id>` to inspect the standard output and error generated by the container. I may use options such as `-f` to follow logs or `--tail` to limit the output. For production workloads managed by Kubernetes, I normally use Kubernetes logging mechanisms and centralized logging rather than depending only on local Docker logs.

---

## 28. What is the difference between a Docker image and container?

A Docker image is an immutable package containing the application code, dependencies, libraries, and filesystem layers required to create a container. A container is a running or stopped instance created from that image. I can create multiple containers from the same image, with each container having its own runtime state while sharing the underlying image layers.

---

# ⚙️ Jenkins / CI-CD

## 29. Your Jenkins pipeline is failing. How will you troubleshoot it?

I start by identifying the exact stage and error message where the pipeline failed rather than rerunning it blindly. I check the Jenkins console logs, recent code changes, agent availability, credentials, environment variables, dependency or Maven failures, Docker build issues, and external service connectivity. If the failure is related to deployment, I separately validate the artifact, image, Kubernetes resources, and deployment events. Once I identify the root cause, I fix it, rerun the appropriate stage or pipeline, and verify the final deployment.

---

## 30. What happens if a Jenkins agent goes offline during a build?

The build can become interrupted or remain waiting depending on the stage and pipeline configuration. I first check whether the agent itself is unavailable because of infrastructure, network, resource exhaustion, or a Jenkins agent/service issue. I inspect the Jenkins node status and logs and verify connectivity between the controller and agent. If another suitable agent is available, the workload may be scheduled there depending on the pipeline configuration. For production CI/CD, I prefer reliable and scalable agents rather than depending on a single machine.

---

## 31. How do you securely store credentials in Jenkins?

I use the Jenkins Credentials Store rather than hardcoding passwords, tokens, or keys inside a Jenkinsfile. Credentials can be referenced through credential IDs and injected only into the required pipeline stage. I also follow least-privilege access and avoid printing secrets in console logs. For more mature environments, Jenkins can integrate with external secret-management systems so sensitive credentials are not unnecessarily stored directly in Jenkins.

---

## 32. What is the difference between Declarative and Scripted Pipeline?

Declarative Pipeline uses a structured and predefined syntax with sections such as `pipeline`, `stages`, `steps`, `environment`, and `post`, which makes it easier to read and maintain. Scripted Pipeline is Groovy-based and provides more programming flexibility for complex logic. For most standard CI/CD pipelines, I prefer Declarative Pipeline because it provides a clear structure while still allowing scripting where required.

---

## 33. How do you implement manual approval before production deployment?

I use a Jenkins `input` step between the deployment stages. For example, after successful build, testing, security scanning, and staging deployment, the pipeline pauses and requests approval from an authorized user or group before production deployment. I also configure appropriate permissions and timeout behavior so that an unattended pipeline does not remain blocked indefinitely.

---

## 34. How do you handle rollback if a production deployment fails?

My rollback approach depends on the deployment strategy. With Kubernetes Deployments, I can use the previous deployment revision and perform a rollout undo. If the issue is related to the application image, I can redeploy the previously known-good image. Before rollback, I assess the impact and confirm the previous version. After rollback, I monitor application health, logs, metrics, and user-facing traffic to ensure service has actually recovered.

---

## 35. How do you pass environment-specific variables in a pipeline?

I keep environment-specific configuration separate from the pipeline's core logic. Jenkins parameters, environment blocks, credentials, configuration files, or external configuration systems can be used depending on the requirement. For Kubernetes deployments, I commonly manage environment-specific values through Helm values files or environment-specific manifests. Sensitive values are always handled through credentials or secret-management mechanisms rather than being committed to Git.

---

## 36. What will you do if the pipeline is successful but the application is not deployed correctly?

A successful Jenkins pipeline only confirms that the configured pipeline steps completed successfully; it does not necessarily prove that the application is healthy. I first verify 
